...
export async function handlePullRequestEvent(context) {
    ...
    try {
        ...
        const parsedFeatures = await parser.parse(repoPath);
        //store in vector db
        ...
        if (!limitCheck.passed) {
            ...
            if (shouldSkipReview(diff)) {
                await githubCommentsHandler.postReview(context, {
        ...
        // Apply token budget
        const { diff: truncatedDiff, rag: truncatedRag, budget } = prepareInputsWithBudget(diff.content, rawRagContext);
